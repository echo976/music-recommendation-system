...
def load_dataset(verbose=0, mode=None, datasetSize=1.0):
    ...
    if mode=="Train":
        ...
        filenames = [os.path.join("SlicedImagesForTrain", f) for f in os.listdir("SlicedImagesForTrain")
                       if f.endswith(".jpg")]
        images_all = [None]*(len(filenames))
        labels_all = [None]*(len(filenames))
        for f in filenames:
            index = int(re.search('SlicedImagesForTrain/(.+?)_.*.jpg', f).group(1))
            genre_variable = re.search('SlicedImagesForTrain/.*_(.+?).jpg', f).group(1)
    ...
    if mode=="Test":
        if(verbose > 0):
            print "Compiling Training and Testing Sets ..."
        filenames = [os.path.join("SlicedImagesForTest", f) for f in os.listdir("SlicedImagesForTest")
                       if f.endswith(".jpg")]
        images = []
        labels = []
        for f in filenames:
            song_variable = re.search('SlicedImagesForTest/.*_(.+?).jpg', f).group(1)
